...
import { On, EventSubscriber } from '@lib/simple-event-bus/event-handler.decorator';
...
describe('EventBus usage', () => {
  [
    ['with SimpleEventBusModule.forRoot()', () => SimpleEventBusModule.forRoot()],
    ['with SimpleEventBusModule.register()', () => SimpleEventBusModule.register()],
    ...
    describe(description, () => {
      ...
      it('calls the handlers with the correct event and eventName', async () => {
        ...
        expect(subscriber.spy.singleEventHandler).toHaveBeenCalledWith('Hello world!');
        expect(subscriber.spy.singleEventHandlerTriggerOtherEvent).toHaveBeenCalledWith('Hello world!');
        expect(subscriber.spy.multipleEventHandlers).toHaveBeenCalledTimes(2);
        expect(subscriber.spy.multipleEventHandlers).toHaveBeenCalledWith('Hello world!', 'myOtherEvent');
        expect(subscriber.spy.multipleEventHandlers).toHaveBeenCalledWith('Hello world!', 'myEvent');
        expect(subscriber.spy.multipleEventHandlersWithPriority).toHaveBeenCalledTimes(2);
        expect(subscriber.spy.multipleEventHandlersWithPriority).toHaveBeenCalledWith('Hello world!', 'myEvent');
        expect(subscriber.spy.multipleEventHandlersWithPriority).toHaveBeenCalledWith('Hello world!', 'myOtherEvent');
        expect(subscriber.spy.singleAsyncEventHandler).toHaveBeenCalledWith('Hello world!', 'myEvent');
      });
...
describe('EventBus service scope', () => {
  describe('can resolve dependencies', () => {
    [
      ['from global module with SimpleEventBusModule.register({ isGlobal: true })', () => SimpleEventBusModule.register({ isGlobal: true })],
      ['from global module with SimpleEventBusModule.forRoot()', () => SimpleEventBusModule.forRoot()],
    ].forEach(([description, simpleEventModuleFactory]: [string, () => any]) => {
      describe(description, () => {
...
describe('Multiple EventBus instances with SimpleEventBusModule.register()', () => {
  ...
  @Module({
    imports: [SimpleEventBusModule.register({ eventBusToken: EVENT_BUS_2_TOKEN })],
  ...
  it('calls the handlers of the subscriber 1 and not the subscriber 2 when using the default event bus', async () => {
    ...
    expect(subscriber1.spy.singleEventHandlerTriggerOtherEvent).toHaveBeenCalledTimes(1);
    expect(subscriber1.spy.multipleEventHandlers).toHaveBeenCalledTimes(2);
    expect(subscriber1.spy.multipleEventHandlersWithPriority).toHaveBeenCalledTimes(2);
  ...
  it('calls the handlers of the subscriber 2 and not the subscriber 1 when using the event bus 2', async () => {
    ...
    expect(subscriber1.spy.singleEventHandlerTriggerOtherEvent).not.toHaveBeenCalled();
    expect(subscriber1.spy.multipleEventHandlers).not.toHaveBeenCalled();
    expect(subscriber1.spy.multipleEventHandlersWithPriority).not.toHaveBeenCalled();
