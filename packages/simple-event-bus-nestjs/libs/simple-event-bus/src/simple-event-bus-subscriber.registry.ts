...
import { EventSubscriber, On, OnEventOptions, OnEventParams } from "./event-handler.decorator";
...
type ComputedEventEntry = { eventBusToken: string | symbol, eventName: string, priority: number };
...
export class SimpleEventBusSubscriberRegistry {
  ...
  onModuleInit() {
    ...
    for (const [eventBusToken, handlers] of Object.entries(entriesByEventBusToken)) {
      const realToken = this.tokenInjectionRecord[eventBusToken] ?? eventBusToken;
  ...
  getEntriesByEventBusToken(): Record<string | symbol, EventMapEntryType[]> {
    ...
    for (const wrapper of handlers) {
      const methodNames = this.metadataScanner.getAllMethodNames(wrapper.metatype.prototype);
  ...
  private prepareEntriesOrThrowError(
    ...
    let defaultEventBusToken = this.convertInjectionTokenToString(DEFAULT_SIMPLE_EVENT_BUS_TOKEN);
    if (!Array.isArray(events)) {
      if (typeof events === 'string') {
        computedEvents = [{ eventName: events, priority: 0, eventBusToken: defaultEventBusToken }];
      } else if (events instanceof Object) {
        computedEvents = Object.entries(events).map(([eventName, priority]) =>
          ({ eventName, priority, eventBusToken: defaultEventBusToken }));
      } else {
      ...
      if (typeof events[1] === 'string') {
        computedEvents = (events as string[]).map((eventName: string) =>
          ({ eventName, priority: 0, eventBusToken: defaultEventBusToken }));
      } else if (typeof events[0] === 'string') {
        const opt = events[1] as OnEventOptions;
        let eventBusToken = this.convertInjectionTokenToString(
          opt?.token ?? DEFAULT_SIMPLE_EVENT_BUS_TOKEN,
        );
        computedEvents = [{ eventName: events[0], priority: opt?.priority ?? 0, eventBusToken }];
  ...
  private convertInjectionTokenToString(token: InjectionToken): string | symbol {
